...
Svelte에서 애플리케이션은 하나 이상의 컴포넌트로 이루어져요. 컴포넌트는 HTML, CSS, JavaScript를 한 곳에 모아놓은 재사용 가능한 코드 덩어리예요. `.svelte` 파일에 작성하면 돼요. 오른쪽 코드 에디터에 열려있는 `App.svelte` 파일이 바로 간단한 컴포넌트 예시예요.
